...
-- ALTER date: 13/12/2016
-- Description:	Procedure para alterar Cancelamento de Reserva 
-- ==============================================================

CREATE PROCEDURE [dbo].[PR_RESERVA_CANCELAMENTO_ALTERAR]
...
begin
	...
	begin transaction
		...
		update reserva.tb_cancelamento 
		set
			id_fonte = @id_fonte
		...
		,	nr_cancelamento_prodesp = @nr_cancelamento_prodesp
		,	nr_cancelamento_siafem_siafisico = @nr_cancelamento_siafem_siafisico
		,	cd_obra = @cd_obra
		,	nr_oc = @nr_oc
		,	cd_ugo = @cd_ugo
		,	cd_uo = @cd_uo
		...
		,	dt_emissao_cancelamento = @dt_emissao_cancelamento
