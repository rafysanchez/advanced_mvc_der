...
-- exec PR_CANCELAMENTO_CONSULTAR_GRID 
-- ==============================================================  
  
CREATE PROCEDURE [dbo].[PR_RESERVA_CANCELAMENTO_CONSULTAR_GRID]  
 --  @id_cancelamento      int = null  ,
  ...
  ,@cd_obra      int = null  
  ,@dt_emissao_cancelamentoDe   date = null  
  ,@dt_emissao_cancelamentoAte   date = null 
 ...
 SELECT Rf.[id_cancelamento]  
	   ,Rf.[cd_processo]    
	   ,Rf.[nr_cancelamento_prodesp]
	   ,Rf.[nr_cancelamento_siafem_siafisico]  
	   ,Rf.[id_programa] 
	   ,Rf.[id_estrutura]  
   ...
   FROM [reserva].[tb_cancelamento]  Rf (nolock)
   ...
   where 
		  ...
		  (@nr_cancelamento_prodesp = Rf.nr_cancelamento_prodesp OR ISNULL(@nr_cancelamento_prodesp,'') = '') AND  
		  (@nr_cancelamento_siafem_siafisico = Rf.nr_cancelamento_siafem_siafisico OR ISNULL(@nr_cancelamento_siafem_siafisico,'') ='') AND  
		  ...
		  (Rf.dt_cadastramento  >= @dt_emissao_cancelamentoDe or @dt_emissao_cancelamentoDe is null)  AND
          (Rf.dt_cadastramento  <= @dt_emissao_cancelamentoAte or @dt_emissao_cancelamentoAte is null)    
