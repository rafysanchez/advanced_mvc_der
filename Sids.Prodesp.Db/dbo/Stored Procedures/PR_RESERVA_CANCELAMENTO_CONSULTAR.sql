...
-- carlos henrique  
-- Description: Procedure para Consultar Cancelamento cadastrado  
-- ==============================================================  
  
CREATE PROCEDURE [dbo].[PR_RESERVA_CANCELAMENTO_CONSULTAR]  
...
begin  
 ...
 SELECT [id_cancelamento]
  ...
  FROM [reserva].[tb_cancelamento]
  
   where (@id_cancelamento = id_cancelamento OR ISNULL(@id_cancelamento,0) = 0) AND  
		 ...
		 (@nr_cancelamento_prodesp = nr_cancelamento_prodesp	 OR ISNULL(@nr_cancelamento_prodesp, '') = '') AND  
		 (@nr_cancelamento_siafem_siafisico = nr_cancelamento_siafem_siafisico OR ISNULL(@nr_cancelamento_siafem_siafisico, '') = '') AND  
		 ...
		 (@dt_emissao_cancelamento = dt_emissao_cancelamento OR ISNULL(@dt_emissao_cancelamento,'') = '') AND
		 ((@ds_status_siafem_siafisico = ds_status_siafem_siafisico OR ISNULL(@ds_status_siafem_siafisico,0) = 0) OR  
         (@ds_status_prodesp = ds_status_prodesp OR ISNULL(@ds_status_prodesp,'') = '')) AND
		 (@ds_status_documento = ds_status_documento OR ISNULL(@ds_status_documento,'') ='') AND
