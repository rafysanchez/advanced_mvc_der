...
namespace Sids.Prodesp.Core.Services.LiquidacaoDespesa
{
    ...
    public class RapRequisicaoService : CommonService
    {
        private readonly ICrudRapRequisicao _repository;
        private readonly ProdespLiquidacaoDespesaService _prodesp;
        private readonly SiafemLiquidacaoDespesaService _siafem;
        private readonly RapRequisicaoNotaService _notas;
        private readonly ChaveCicsmoService _chave;
        


        public RapRequisicaoService(ILogError log, ICommon common, IChaveCicsmo chave,
            ICrudRapRequisicao repository, ICrudRapRequisicaoNota notas,
            ...
            : base(log, common, chave)
        {
            ...
            _notas = new RapRequisicaoNotaService(log, notas);
        ...
        public AcaoEfetuada Excluir(RapRequisicao entity, int recursoId, short action)
        {
            try
            {
                ...
                if (recursoId > 0) return LogSucesso(action, recursoId, $"RapRequisicao : Codigo {entity.Id}");
        ...
        public RapRequisicao Selecionar(int Id)
        {
            var entity = _repository.Get(Id);
            entity.Notas = _notas.Buscar(new RapRequisicaoNota { SubempenhoId = entity.Id });
        ...
        {
            var empenhos = new List<RapRequisicao>();
            ...
            if (empenhosErros.Count > 0)
                if (empenhos.Count == 1)
                    ...
                    result += Environment.NewLine + "; / Algumas Requisições de Rap não puderam ser transmitidas";
            return result;
        }
