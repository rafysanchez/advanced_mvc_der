namespace Sids.Prodesp.UI.Areas.Movimentacao.Models
{
    ...
    public class ValorMovimentacao2ViewModel
    {
        ...
        public ValorMovimentacao2ViewModel CreateInstance(int modelId, IEnumerable<MovimentacaoMes> meses)
        {
            var dictionary = meses
                .Where(w => w.IdDistribuicao == modelId)
                .Select(s => new { s.Descricao, s.ValorMes })
                .ToDictionary(d => d.Descricao, d => d.ValorMes);

            return dictionary.Any() ?
                new ValorMovimentacao2ViewModel()
                {
